...
def accuracy(typed, reference):
    ...
    correct = 0
    n = 0
    while n < min(len(typed_words), len(reference_words)):
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def count(c_start, c_goal, c):
        if limit < c:
            return limit + 1
        if not c_start and not c_goal:
            return c
        elif not c_start or not c_goal:
            return count(c_start[1:], c_goal[1:], c+1)
        elif c_start[0] == c_goal[0]:
            return count(c_start[1:], c_goal[1:], c)
        else:
            return count(c_start[1:], c_goal[1:], c+1)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    print_progress(user_id, progress)
    return progress

def print_progress(user_id, progress):
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        difference.append([t[i] - t[i-1] for i in range(1, len(t))])
...
def fastest_words(match):
    ...
    for w in word_indices:
        faster_player = 0
        fast_t = time(match, faster_player, w)
        for p in player_indices:
            player_t = time(match, p, w)
            if player_t < fast_t:
                faster_player = p
                fast_t = player_t
        result[faster_player].append(word_at(match, w))
    return result
    # END PROBLEM 10
