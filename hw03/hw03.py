...
def count_coins(change):
    ...
    "*** YOUR CODE HERE ***"
    current = 1
    return helper(change, current)
    
def helper(change, current):
    if change == 0:
        return 1
    elif change < 0 or current == None:
        return 0
    else:
        return helper(change - current, current) + helper(change, ascending_coin(current))
