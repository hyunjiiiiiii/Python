...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search('[Cc][Ss][0-9]+[ABCabc]*|[Cc][Ss]\s[0-9]+[ABCabc]*', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'(?:(?:[0-1]?[0-9]|[2][0-3]):(?:[0-5][0-9])(?:(?:\s?)[AaPp][Mm])?)', text)
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r'(\([0-9]{3}\)|[0-9]{3})[ -\.]?[0-9]{3}[ -\.]?[0-9]{4}', text)
...
def most_common_code(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    num = re.findall(r'(\([0-9]{3}\)|[0-9]{3})[ -\.]?[0-9]{3}[ -\.]?[0-9]{4}', text)
