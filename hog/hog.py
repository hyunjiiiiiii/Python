...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            player0 = take_turn(strategy0(score0, score1), score1, dice, goal)
            score0 += player0
            score0 += hog_pile(score0, score1)
            
        elif who == 1:
            player1 = take_turn(strategy1(score1, score0), score0, dice, goal)
            score1 += player1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0, score1):
        if who == 0:
            gain_score = score0 - last_score
        else:
            gain_score = score1 - last_score
        if gain_score > running_high:
            print(gain_score, "point(s)! That's a record gain for Player", who, "\b!")
            new_last_score = last_score + gain_score
            new_running_high = gain_score
            return announce_highest(who, new_last_score, new_running_high)
        else:
            new_last_score = last_score + gain_score
            return announce_highest(who, new_last_score, running_high)
    return announce
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if opponent_score == score + picky_piggy(opponent_score):
        return 0
    elif score + picky_piggy(opponent_score) == 2 * opponent_score:
        return num_rolls
    else:
